...
func (c *Config) Validate() error {
    ...
    if err != nil || port < 1 || port > 65535 {
        return fmt.Errorf("无效的端口号: %s", c.Port)
    }

    // 验证 API_ROOT 格式
    if c.APIRoot != "" && !strings.HasPrefix(c.APIRoot, "/") {
        return fmt.Errorf("API_ROOT 必须以 / 开头: %s", c.APIRoot)
    }
...
### 2. 添加配置文件支持（可选）
如果需要配置文件，建议使用 [Viper](https://github.com/spf13/viper)：
```go
import "github.com/spf13/viper"

func Load() *Config {
    viper.SetDefault("PORT", "8088")
    viper.SetDefault("API_ROOT", "")
    viper.SetDefault("DATA_DIR", "./data")

    viper.AutomaticEnv()
    viper.SetEnvPrefix("COOKIECLOUD")
